...
library(leaps)  # version 3.1 (Miller, 2022)

# Read the data from Excel file
# Note: You may need to install the readxl package if not already installed
if (!require(readxl)) {
...
coef_table <- data.frame(
  ...
  significant_predictors <- coef_table[coef_table$p_value < 0.05, ]
  ...
  write.csv(coef_table, "polynomial_regression_coefficients.csv", row.names = FALSE)
  
  # Write model summary statistics to CSV
  model_stats <- data.frame(
    Metric = c("R-squared", "Adj R-squared", "F-statistic", "p-value (F-test)", "AIC", "BIC", "RMSE", "MAE", "N"),
    Value = c(
      summary(poly_model)$r.squared,
      summary(poly_model)$adj.r.squared,
      summary(poly_model)$fstatistic[1],
      pf(summary(poly_model)$fstatistic[1], 
         summary(poly_model)$fstatistic[2], 
         summary(poly_model)$fstatistic[3], 
         lower.tail = FALSE),
      AIC(poly_model),
  ...
  write.csv(model_stats, "polynomial_regression_model_stats.csv", row.names = FALSE)
  
  cat("Results exported to:\n")
  cat("  - polynomial_regression_coefficients.csv\n")
  cat("  - polynomial_regression_model_stats.csv\n")
  
  # Save the model for future use
  saveRDS(poly_model, "polynomial_regression_model.rds")
  cat("Model saved as: polynomial_regression_model.rds\n")
  
  # Optional: Create a function to make predictions with the model
  predict_fyld <- function(NDVI, PLTHT, CHL, ANGBR, STMDI, LODG, NOHAV, PPSTD) {
    new_data <- data.frame(
      NDVI = NDVI,
      PLTHT = PLTHT,
      CHL = CHL,
    ...
    prediction <- predict(poly_model, newdata = new_data)
    return(prediction)
  }
  
  cat("\nPrediction function 'predict_fyld()' created.\n")
  cat("Example usage: predict_fyld(NDVI = 0.8, PLTHT = 100, CHL = 50, ANGBR = 75, STMDI = 20, LODG = 1, NOHAV = 5, PPSTD = 2)\n")
